...
public class DigitalCellsClass extends UpdatableCollector {
    ...
    public DigitalCellsClass(MainFile mainFile){
        ...
        cells = new CellWrapper.Builder()
                .add(mainFile,"cell1",
                        colorBuilder.initialize(mainFile, expansionHubDevices.getI2C(0)).setFields(0.229, 0.297, 0.297, 0.0).get(),
                        colorBuilder.initialize(mainFile, expansionHubDevices.getI2C(1)).setFields(0.229, 0.263, 0.263, 0.0).get()
                )
                .add(mainFile,"cell2",
                        colorBuilder.initialize(mainFile, expansionHubDevices.getI2C(2)).setFields(0.186, 0.203, 0.203, 0.0).get(),
                        colorBuilder.initialize(mainFile, expansionHubDevices.getI2C(3)).setFields(0.150, 0.203, 0.203, 0.0).get())
                .add(mainFile,"cell3",
                        colorBuilder.initialize(mainFile, controlHubDevices.getI2C(0)).setFields(0.161, 0.195, 0.195, 0.0).get(),
                        colorBuilder.initialize(mainFile, controlHubDevices.getI2C(1)).setFields(0.161, 0.186, 0.195, 0.0).get());

        servosCollector
                .add(servoBuilder.initialize(mainFile, pusher0).setFields(1000.0, 180.0).get())
                .add(servoBuilder.initialize(mainFile, pusher2).setFields(750.0, 180.0).get())
                .add(servoBuilder.initialize(mainFile, pusher1).setFields(1000.0, 180.0).get());
    ...
    public static class CellWrapper extends UpdatableCollector {
        ...
        public String name;
        public CellWrapper(MainFile mainFile, String cellName, ColorSensorWrapper...sensorsWrapperIn){
            super(mainFile);
            this.name = cellName;
        ...
        public static class Builder{
            ...
            public Builder add(MainFile mainFile, String cellName, ColorSensorWrapper...sensors){
                cells.put(cellName, new CellWrapper(mainFile, cellName, sensors));
                return this;
            }

...
public class Task {
    ...
    public Task(double targetHeight, double targetPower, HoodedShoter teleSkope, Telemetry telemetry, int queuePlace){
        Args.LiftArgs liftArgs = new Args.LiftArgs(targetHeight, targetPower);

        this.queuePlace = queuePlace;
    }

    public Task(String targetName, double targetPos, HoodedShoter teleSkope, Telemetry telemetry, int queuePlace){
        Args.ServoArgs servoArgs = new Args.ServoArgs(targetName, targetPos);

        this.queuePlace = queuePlace;
    }

    public Task(double targetHeight, double targetPower, String targetName, double targetPos, HoodedShoter teleSkope, Telemetry telemetry, int queuePlace){
        Args.LiftArgs liftArgs = new Args.LiftArgs(targetHeight, targetPower);
        Args.ServoArgs servoArgs = new Args.ServoArgs(targetName, targetPos);
